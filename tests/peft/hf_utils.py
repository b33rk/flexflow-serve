...
def prepare_model_for_lora_finetuning(model, save_peft_tensors=False, use_full_precision=True):
    # Freeze all layers except the LORA ones. Cast small layers to the appropriate precision for stability
...
def build_peft_model(args, peft_config):
    ...
    model = PeftModel.from_pretrained(model, 
                                      ...
                                      config=peft_config, 
                                      torch_dtype=torch.float32 if args.use_full_precision else torch.float16)
    model = prepare_model_for_lora_finetuning(model, args.save_peft_tensors, args.use_full_precision)
    return model
