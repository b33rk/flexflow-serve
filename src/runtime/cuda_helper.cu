...
torch::Dtype getTorchDtype() {
  if constexpr (std::is_same_v<DT, at::Half> || std::is_same_v<DT, half>) {
    ...
    static_assert(!std::is_same_v<DT, DT>,
                  "Unsupported datatype. Only fp16 (torch::Half), bf16 (torch::BFloat16), CUDA half, float, double, int32 and int64 are supported.");
  }
}
...
template torch::Tensor createTorchTensorFromCuda<float>(void* cudaData, const std::vector<int64_t>& dims);
template torch::Tensor createTorchTensorFromCuda<half>(void* cudaData, const std::vector<int64_t>& dims);
template torch::Tensor createTorchTensorFromCuda<double>(void* cudaData, const std::vector<int64_t>& dims);
template torch::Tensor createTorchTensorFromCuda<int32_t>(void* cudaData, const std::vector<int64_t>& dims);
template torch::Tensor createTorchTensorFromCuda<int64_t>(void* cudaData, const std::vector<int64_t>& dims);
template torch::Tensor createTorchTensorFromCuda<float>(const void* cudaData, const std::vector<int64_t>& dims);
template torch::Tensor createTorchTensorFromCuda<half>(const void* cudaData, const std::vector<int64_t>& dims);
template torch::Tensor createTorchTensorFromCuda<double>(const void* cudaData, const std::vector<int64_t>& dims);
template torch::Tensor createTorchTensorFromCuda<int32_t>(const void* cudaData, const std::vector<int64_t>& dims);
template torch::Tensor createTorchTensorFromCuda<int64_t>(const void* cudaData, const std::vector<int64_t>& dims);
